...
func (mounter *Mounter) MountSensitive(
	...
	if bind {
		err := mounter.doMount(
			...
			fstype,
			bindOpts,
			bindRemountOptsSensitive,
		)
		if err != nil {
			return err
		}
		return mounter.doMount(
			mounterPath,
			defaultMountCommand,
			source,
			target,
...
// sensitiveOptions is an extension of options except they will not be logged (because they may contain sensitive
// material).
func (mounter *Mounter) doMount(
	mounterPath string,
	mountCmd string,
	source string,
	target string,
	...
	if mounter.withSystemd {
		...
		mountCmd, mountArgs, mountArgsLogStr = AddSystemdScopeSensitive(
			"systemd-run",
			target,
			mountCmd,
			mountArgs,
...
func MakeMountArgsSensitive(
	...
	options []string,
	sensitiveOptions []string,
) (mountArgs []string, mountArgsLogStr string) {
...
func AddSystemdScopeSensitive(
	systemdRunPath, mountName, command string,
	args []string,
	mountArgsLogStr string,
...
func (mounter *SafeFormatAndMount) formatAndMountSensitive(
	source string,
	target string,
	fstype string,
	options []string,
	...
	if existingFormat == "" {
		...
		if readOnly {
			return NewMountError(
				UnformattedReadOnly,
				"cannot mount unformatted disk %s as we are manipulating it in read-only mode",
				source,
			)
		}
		...
		if err != nil {
			...
			detailedErr := fmt.Sprintf(
				"format of disk %q failed: type:(%q) target:(%q) options:(%q) errcode:(%v) output:(%v) ",
				source,
				fstype,
				target,
				sensitiveOptionsLog,
