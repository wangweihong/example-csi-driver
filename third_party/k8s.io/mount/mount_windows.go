...
func (mounter *Mounter) MountSensitive(
	...
	if source == "tmpfs" {
		klog.V(3).
			Infof("mounting source (%q), target (%q), with options (%q)", source, target, sanitizedOptionsForLogging)
...
func newSMBMapping(username, password, remotepath string) (string, error) {
	if username == "" || password == "" || remotepath == "" {
		return "", fmt.Errorf(
			"invalid parameter(username: %s, password: %s, remoteapth: %s)",
			username,
...
func (mounter *SafeFormatAndMount) formatAndMountSensitive(
	...
	klog.V(2).
		Infof("formatAndMount disk(%s) fstype(%s) on(%s) with output(%s) successfully", driverPath, fstype, target, string(output))
	return nil
}
...
func listVolumesOnDisk(diskID string) (volumeIDs []string, err error) {
	...
	if err != nil {
		return []string{}, fmt.Errorf(
			"error list volumes on disk. cmd: %s, output: %s, error: %v",
			cmd,
			string(output),
			err,
		)
