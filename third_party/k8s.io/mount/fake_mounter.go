...
func (f *FakeMounter) MountSensitive(
	source string,
	target string,
	fstype string,
	options []string,
	...
	for _, option := range options {
		// find 'bind' option
		if option == "bind" {
			// This is a bind-mount. In order to mimic linux behavior, we must
	...
	f.MountPoints = append(
		f.MountPoints,
		MountPoint{Device: source, Path: absTarget, Type: fstype, Opts: append(opts, sensitiveOptions...)},
	)
