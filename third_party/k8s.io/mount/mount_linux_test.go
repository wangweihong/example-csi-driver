...
func mountPointsEqual(a, b *MountPoint) bool {
	if a.Device != b.Device || a.Path != b.Path || a.Type != b.Type || !reflect.DeepEqual(a.Opts, b.Opts) ||
		a.Pass != b.Pass ||
		a.Freq != b.Freq {
		return false
	}
...
func TestGetDeviceNameFromMount(t *testing.T) {
	...
	for i, test := range tests {
		if device, refs, err := GetDeviceNameFromMount(fm, test.mountPath); err != nil || test.expectedRefs != refs ||
			test.expectedDevice != device {
			t.Errorf(
				"%d. GetDeviceNameFromMount(%s) = (%s, %d), %v; expected (%s,%d), nil",
				i,
				test.mountPath,
				device,
				refs,
...
func TestGetMountRefsByDev(t *testing.T) {
	...
	for i, test := range tests {
		...
		if refs, err := getMountRefsByDev(fm, test.mountPath); err != nil || !setEquivalent(test.expectedRefs, refs) {
			t.Errorf(
				"%d. getMountRefsByDev(%q) = %v, %v; expected %v, nil",
				i,
				test.mountPath,
				refs,
				err,
...
`,
			[]string{
				"/var/lib/kubelet/pods/1890aef5-5a60-11e8-962f-000c29bb0377/volumes/kubernetes.io~local-volume/local-pv-test",
			},
			nil,
		},
		{
...
`,
			[]string{
				"/var/lib/kubelet/pods/e25f2f01-5b06-11e8-8694-000c29bb0377/volumes/kubernetes.io~local-volume/local-pv-test",
			},
			nil,
		},
		{
...
`,
			[]string{
				"/var/lib/kubelet/pods/f30dc360-5a5d-11e8-962f-000c29bb0377/volumes/kubernetes.io~local-volume/local-pv-1fb30a1c",
			},
			nil,
		},
		{
...
func TestSensitiveMountOptions(t *testing.T) {
	...
	for _, v := range testcases {
		// Act
		mountArgs, mountArgsLogStr := MakeMountArgsSensitive(
			v.source,
			v.target,
			v.fstype,
			v.options,
		...
		for _, option := range v.options {
			...
			if !strings.Contains(mountArgsLogStr, option) {
				t.Errorf(
					"Expected option (%q) to exist in returned mountArgsLogStr (%q), but it does",
					option,
					mountArgsLogStr,
				)
			}
		...
		for _, sensitiveOption := range v.sensitiveOptions {
			if found := contains(mountArgs, sensitiveOption, t); !found {
				t.Errorf(
					"Expected sensitiveOption (%q) to exist in returned mountArts (%q), but it does not",
					sensitiveOption,
					mountArgs,
				)
			}
			if strings.Contains(mountArgsLogStr, sensitiveOption) {
				t.Errorf(
					"Expected sensitiveOption (%q) to not exist in returned mountArgsLogStr (%q), but it does",
					sensitiveOption,
					mountArgsLogStr,
				)
			}
