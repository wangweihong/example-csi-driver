...
func TestMakeBindOptsSensitive(t *testing.T) {
	...
	for _, test := range tests {
		bind, bindOpts, bindRemountOpts, bindRemountSensitiveOpts := MakeBindOptsSensitive(
			test.mountOptions,
			test.sensitiveMountOptions,
		)
		if bind != test.isBind {
			t.Errorf("Expected bind to be %v but got %v", test.isBind, bind)
		}
		if test.isBind {
			...
			if !reflect.DeepEqual(test.expectedSensitiveRemountOpts, bindRemountSensitiveOpts) {
				t.Errorf(
					"Expected sensitive remount options to be %+v got %+v",
					test.expectedSensitiveRemountOpts,
					bindRemountSensitiveOpts,
				)
			}
		}
...
func TestOptionsForLogging(t *testing.T) {
	...
	for _, v := range testcases {
		...
		if actualCount != expectedCount {
			t.Errorf(
				"Found %v instances of %q in %q. Expected %v",
				actualCount,
