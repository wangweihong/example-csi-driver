...
func TestPathWithinBase(t *testing.T) {
	...
	for _, test := range tests {
		...
		assert.Equal(
			...
			result,
			test.expectedResult,
			"Expect result not equal with PathWithinBase(%s, %s) return: %q, expected: %q",
			test.fullPath,
			test.basePath,
			result,
			test.expectedResult,
...
func TestIsLikelyNotMountPoint(t *testing.T) {
	...
	for _, test := range tests {
		...
		assert.Equal(
			...
			result,
			test.expectedResult,
			"Expect result not equal with IsLikelyNotMountPoint(%s) return: %q, expected: %q",
			filePath,
			result,
			test.expectedResult,
		)
	...
	for _, test := range tests {
		...
		if test.expectError {
			assert.NotNil(
				t,
				err,
				"Expect error during newSMBMapping(%s, %s, %s, %v)",
				test.username,
				test.password,
				test.remotepath,
			)
